...
void VariantManager::setValue(QtProperty *property, const QVariant &val)
{
    if (theValues.contains(property)) {
#if 1
        bool canConvert = !(val.type() != QVariant::String && !val.canConvert(QVariant::String));
        ...
        if (val.type() == QVariant::UserType)
            str = val.value<FileString>().filestring;
        else
        ...
        if (val.type() != QVariant::String && !val.canConvert(QVariant::String))
            return;
        FileString str = val.value<FileString>();
        Data d = theValues[property];
        if (d.value == str.filestring)
            return;
        d.value = str.filestring;
        theValues[property] = d;
        emit propertyChanged(property);
        emit valueChanged(property, str);
        return;
